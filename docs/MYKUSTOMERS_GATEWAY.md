...
Connected validation found direct console logging in libsignal 6.0.0 that bypassed Baileys' silent logger and printed Signal session objects. `patches/libsignal+6.0.0.patch` removes lifecycle object dumps, redacts session inspection, and removes raw decryption-error/bucket dumps. npm postinstall applies this version-pinned patch; the focused lifecycle test verifies no synthetic key material is logged. Review and port this patch whenever upgrading libsignal. Earlier test-time journal entries remain sensitive and restricted to server administrators under the existing bounded retention policy; they are not copied into reports. No customer messages were involved.
