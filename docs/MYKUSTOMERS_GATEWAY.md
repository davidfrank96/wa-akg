...
7. Before authorized DNS and TLS, install `nginx-http.conf` as the default server: only `/healthz` is public, all other HTTP paths return 426. Do not expose credentials or pairing via plaintext HTTP. Once the user creates the authorized A record, configure certificate-verified HTTPS before enabling the API at the proxy.
